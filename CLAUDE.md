...
2. ✅ **완료(2026-09-12)** — `가격예측_변동성_일일수집.py` 최초 1회 수동 실행, 100/100종목
   ...
   아래 절 참고
3. 크론/스케줄러 등록 — ✅ **부분 완료(2026-09-20)**. DB를 클라우드(Neon)로 이전하고
   ...
   시작한다. `daily_pipeline.bat`(로컬 학습)의 Windows 작업 스케줄러 등록은 사용자가 직접
   완료(MLflow 서버 자동시작 포함). 상세는 `가격예측/종목_월간갱신.py`/두 워크플로 파일
   참고.
