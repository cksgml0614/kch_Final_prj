...
6. **신규(2026-09-12)** `actual_volatility` 백필 스크립트 작성 — 코드베이스 어디에도 구현이
   없음을 확인함(주석에 "추후 백필 작업"으로만 언급돼 있었음). 다음 작업 후보로 명시
7. **신규(2026-09-12)** 며칠 더 수동 실행하며 배포 게이트 판정 추이 관찰 — 최초 실행에서
   ...
   ⚠️ **2026-09-14부터 이 관찰이 실질적 의미를 갖게 됨** — 게이트 강제 적용(아래 "배포
   게이트 강제 적용 + 예측값 sanity check" 절)으로 미통과 시 `model_predictions`에 그날 행이
   아예 안 쌓인다. 미통과가 여러 날 이어지면 관찰할 신규 데이터 자체가 끊긴다는 뜻이므로,
   이 경우 원인 규명(위 (a)~(c) 가설) 우선순위를 올려 재검토할 것
...
**게이트가 이제 실제로 저장을 막기 때문에, 이 패턴이 계속되면 `model_predictions`에
